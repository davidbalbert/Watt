...
final class BufferTests: XCTestCase {
    func testApplyingMultipleTokens() {
        ...
        XCTAssertEqual(buffer.contents.runs[\.tokenType].count, 19)

        var iter = buffer.contents.runs[\.tokenType].makeIterator()
        var r = iter.next()!
        XCTAssertEqual(r.tokenType, .keyword)
        XCTAssertEqual(Range(intRangeFor: r.range), 0..<8)
        XCTAssertEqual(buffer.text[0..<8], "#include")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 9..<10)
        XCTAssertEqual(buffer.text[9..<10], "<")
        
        r = iter.next()!
        XCTAssertEqual(r.tokenType, .string)
        XCTAssertEqual(Range(intRangeFor: r.range), 10..<17)
        XCTAssertEqual(buffer.text[10..<17], "stdio.h")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 17..<18)
        XCTAssertEqual(buffer.text[17..<18], ">")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .type)
        XCTAssertEqual(Range(intRangeFor: r.range), 20..<23)
        XCTAssertEqual(buffer.text[20..<23], "int")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .function)
        XCTAssertEqual(Range(intRangeFor: r.range), 24..<28)
        XCTAssertEqual(buffer.text[24..<28], "main")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 28..<29)
        XCTAssertEqual(buffer.text[28..<29], "(")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .type)
        XCTAssertEqual(Range(intRangeFor: r.range), 29..<33)
        XCTAssertEqual(buffer.text[29..<33], "void")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 33..<34)
        XCTAssertEqual(buffer.text[33..<34], ")")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 35..<36)
        XCTAssertEqual(buffer.text[35..<36], "{")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .function)
        XCTAssertEqual(Range(intRangeFor: r.range), 41..<47)
        XCTAssertEqual(buffer.text[41..<47], "printf")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 47..<48)
        XCTAssertEqual(buffer.text[47..<48], "(")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .string)
        XCTAssertEqual(Range(intRangeFor: r.range), 48..<65)
        XCTAssertEqual(buffer.text[48..<65], "\"Hello, world!\\n\"")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 65..<66)
        XCTAssertEqual(buffer.text[65..<66], ")")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 66..<67)
        XCTAssertEqual(buffer.text[66..<67], ";")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .keyword)
        XCTAssertEqual(Range(intRangeFor: r.range), 72..<78)
        XCTAssertEqual(buffer.text[72..<78], "return")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .number)
        XCTAssertEqual(Range(intRangeFor: r.range), 79..<80)
        XCTAssertEqual(buffer.text[79..<80], "0")

        r = iter.next()!
        XCTAssertEqual(r.tokenType, .delimiter)
        XCTAssertEqual(Range(intRangeFor: r.range), 80..<81)
        XCTAssertEqual(buffer.text[80..<81], ";")

        // TODO: Broken
        // r = iter.next()!
        // XCTAssertEqual(r.tokenType, .delimiter)
        // XCTAssertEqual(Range(intRangeFor: r.range), 82..<83)
