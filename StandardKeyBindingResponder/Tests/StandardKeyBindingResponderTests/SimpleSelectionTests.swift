...
final class SimpleSelectionTests: XCTestCase {
    func testCreateCaret() {
        ...
        let s = SimpleSelection(caretAt: string.index(at: 1), affinity: .upstream, granularity: .character)
        XCTAssert(s.isCaret)
    ...
    func testCreateDownstreamSelection() {
        let string = "Hello, world!"
        let s = SimpleSelection(anchor: string.index(at: 1), head: string.index(at: 5), granularity: .character)
        XCTAssert(s.isRange)
        XCTAssertEqual(string.index(at: 1), s.lowerBound)
    ...
    func createUpstreamSelection() {
        let string = "Hello, world!"
        let s = SimpleSelection(anchor: string.index(at: 5), head: string.index(at: 1), granularity: .character)
        XCTAssert(s.isRange)
        XCTAssertEqual(string.index(at: 1), s.lowerBound)
        XCTAssertEqual(string.index(at: 5), s.upperBound)
        XCTAssertEqual(.upstream, s.affinity)
